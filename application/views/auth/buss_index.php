...
//添加新分组
function addNew(){
	showFarme('添加平台商','<?=site_url("auth/bussAdd")?>','80%','90%');
}
//修改分组
function group_edit(group_id){
	showFarme('编辑平台商','<?=site_url("auth/bussAdd")?>/'+group_id,'80%','90%');
}
//删除分组
function group_del(group_id){
		layer.confirm('您确定要删除吗？<br>删除后可能会影响该平台下的佣金结算！', {
		    btn: ['确定','取消'] //按钮
		}, function(){
			$.get("<?=site_url("auth/del_buss") ?>/"+group_id,function(data){
				if (data.status == 1){
