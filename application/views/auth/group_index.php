...
//添加新分组
function addNew(){
	showFarme('添加用户分组','<?=site_url("auth/group_add")?>','50%','70%');
}
//修改分组
function group_edit(group_id){
	showFarme('编辑用户分组','<?=site_url("auth/group_add")?>/'+group_id,'50%','70%');
}
//删除分组
function group_del(group_id){
		layer.confirm('您确定要删除吗？<br>删除后可能会影响该组下的用户登录操作！', {
			...
			$.get("<?=site_url("auth/del_group") ?>/"+group_id,function(data){
				if (data.status == 1){
